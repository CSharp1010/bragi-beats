...
void DrawSongQueue(Rectangle queue) {
    int fontSize = 12;
    int startY = queue.y + 10;
    int padding = 5;
    int textHeight = 20;
    SongNode* node = head;

    DrawRectangle(queue.x, queue.y, queue.width, queue.height, OFFWHITE);

    while (node != NULL && startY < queue.y + queue.height) {
        Color textColor = BLACK;
        Rectangle textBackground = { queue.x + 5, startY, queue.width - 10, textHeight };
        ...
        DrawText(node->title, queue.x + 10, startY +(textHeight / 2) - (fontSize / 2), fontSize, textColor);
...
void DrawTextBox(Rectangle textBoxBounds, char* text, int maxLength, bool* isActive) {
    if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
        *isActive = CheckCollisionPointRec(GetMousePosition(), textBoxBounds);
    }
...
void RenderVisualizer(float out_smooth[], float out_phase[], float out_power[], size_t numberOfFftBins, int centerX, int centerY, Rectangle visualizerSpace) {
    switch (currentVisualizer) {
        ...
        case VISUALIZER_CENTER_LINE:
            centerLineVisualizer(out_smooth, out_phase, out_power, numberOfFftBins, visualizerSpace);
            break;
